...
int main()
{
 ...
 for(i=0; i<allcover.size(); i++)
 {
  ...
  if(s<=k)
  {
   outfile<<"Clique ("<<n-s<<"): "; outfile2<<n-s<<" ";
   for(j=0; j<cover.size(); j++) if(cover[j]==0) { outfile<<j+1<<" "; outfile2<<j+1<<" ";}
   outfile<<endl; outfile2<<endl;
